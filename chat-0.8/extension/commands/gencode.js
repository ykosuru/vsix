...
`);
    ...
    // Show sources if any
    if ((files && files.length > 0) || attachedDocNames.length > 0) {
        let sourcesInfo = '';
        if (files && files.length > 0) {
            sourcesInfo += `<details><summary>📂 Reference code (${files.length} files)</summary>\n\n`;
        ...
        if (attachedDocNames.length > 0) {
            sourcesInfo += `<details><summary>📎 Attached docs (${attachedDocNames.length})</summary>\n\n`;
            for (const name of attachedDocNames) {
                sourcesInfo += `- \`${name}\`\n`;
            }
            sourcesInfo += `\n</details>\n\n`;
        }
        response.markdown(sourcesInfo);
    }
    ...
    if (hasPreviousContext && !query.trim()) {
        userPrompt = `Generate Java code based on these requirements from the previous response:

## Requirements/Context from Previous Response
