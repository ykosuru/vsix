...
| Command | Description |
|---------|-------------|
| `/conf.r <pages>` | Read one or more pages (comma-separated) |
| `/conf.w <title>` | Write previous response to Confluence |

### Configuration

Settings in VS Code (Cmd+, → search "astracode.confluence"):

| Setting | Description |
|---------|-------------|
| `baseUrl` | Confluence instance URL (e.g., `https://company.atlassian.net/wiki`) |
| `spaceKey` | Target space key |
| `username` | Email for Atlassian Cloud |
| `apiToken` | API token from id.atlassian.com |
| `parentPageId` | Optional parent page for organization |

### Read Flow

```
@astra /conf.r Architecture, Design Patterns, API Reference
                    ↓
┌──────────────────────────────────────┐
│         confluence.js                 │
│  1. Parse comma-separated page names  │
│  2. For each page:                    │
│     - Find by title or ID             │
│     - Fetch body.storage              │
│     - Convert to markdown             │
...
@astra /deepwiki partition pruning
                    ↓
[Documentation generated in chat]
                    ↓
@astra /conf.w Partition Pruning Docs
                    ↓
┌──────────────────────────────────────┐
│         confluence.js                 │
│  1. Read previous response            │
│  2. Convert markdown → Confluence     │
│  3. Check if page exists              │
│  4. Create or update page             │
│  5. Return URL                        │
└──────────────────────────────────────┘
                    ↓
✅ Published: https://company.atlassian.net/wiki/...
```

### Markdown to Confluence Conversion

The `markdownToConfluence()` function converts:
...
- `GET /rest/api/content?title=X&spaceKey=Y` - Find existing page
...
- No API token → Prompts for input
- No content → Instructs to run command first
- API error → Shows troubleshooting tips
