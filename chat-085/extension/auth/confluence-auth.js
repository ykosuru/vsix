...
class ConfluenceAuth {
    ...
    async getCredentials() {
        ...
        // Check for API token (simple auth) - ensure values are not empty strings
        const apiToken = config.get('apiToken') || '';
        const username = config.get('username') || '';
        const baseUrl = config.get('baseUrl') || '';
        
        // Debug logging
        console.log(`[AstraCode] Confluence config: baseUrl="${baseUrl}", username="${username}", apiToken=${apiToken ? '***' : '(empty)'}`);
        
        if (apiToken.trim() && username.trim() && baseUrl.trim()) {
            console.log(`[AstraCode] Using API token auth for ${baseUrl}`);
            return {
                type: 'basic',
