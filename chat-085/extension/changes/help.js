...
## 📁 The /in Modifier (Folder Filter)

Limit workspace search to specific folders or exclude folders:

\`\`\`
@astra /requirements /source w /in payments-service pacs.008
@astra /logic /in repo1,repo2 validation
@astra /gencode /source w /in src,-test payment handler
\`\`\`

| Pattern | Effect |
|---------|--------|
| \`/in payments\` | Only folders containing "payments" |
| \`/in repo1,repo2\` | Multiple include folders |
| \`/in -docs\` | Exclude folders containing "docs" |
| \`/in src,-test\` | Include src, exclude test |
| \`/in -docs,-generated\` | Exclude multiple folders |

**Use cases:**
\`\`\`
# Focus on code, skip documentation
@astra /requirements /source w /in -docs field mapping

# Search specific repo only  
@astra /logic /in payments-repo validation

# Code only, no tests
@astra /gencode /source w /in src,-test create handler
\`\`\`
